...
class stimulus:
    ...
    def deconvolve(self,systemOutput):
        ...
        if self.type == 'sinesweep':
            ...
            # adapt output length to recorded signal length plus inverse filter length
            rec_len = systemOutput.shape[0]
            tmplen = self.invfilter.shape[0] + rec_len - 1
            RIRs = np.zeros((tmplen, numChans))
            ...
            for idx in range(0, numChans):
                ...
                if self.repetitions == 1:
                    # single sweep: just use everything (including the built-in delays)
                    sig_avg = systemOutput[:, idx]
                else:
                    # multiple sweeps: chop into blocks of length Lp and average
                    sig_reshaped = systemOutput[:, idx].reshape((self.repetitions, self.Lp))
                    sig_avg      = np.mean(sig_reshaped, axis=0)

                # Deconvolution
                deconv = fftconvolve(self.invfilter, sig_avg)
                RIRs[:deconv.shape[0], idx] = deconv
